...
describe("Negative tests", function () {
  it("Should show error in case of registrating without email", function () {
    ...
    expect(() => registrationForm.register()).to.throw("Please specify email and password.");
  });
  ...
  it("Should show error in case of registrating with password without password confirmation", function () {
    ...
    expect(() => registrationForm.register()).to.throw("Passwords do not match.");
  });
  ...
  it("Should show error if password has less then 8 symbols", function () {
    ...
    expect(() => registrationForm.register()).to.throw("Password must be at least 8 characters long.");
  });
  ...
  it("Should register user with password that contains symbols: ~`! @#$%^&*()_-+={[}]|:;\"'<,>.?", function () {
    ...
    expect(registrationForm.password).to.equal("passworD1~`! @#$%^&*()_-+={[}]|:;\"'<,>.?");
    expect(registrationForm.confirmPassword).to.equal("passworD1~`! @#$%^&*()_-+={[}]|:;\"'<,>.?");
